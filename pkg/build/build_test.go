...
func TestBuildRejectsUnsupported(t *testing.T) {
	...
	tests := []struct {
		...
		{"method on a boolean named type", "package main\n\ntype Flag bool\n\nfunc (f Flag) On() bool {\n\treturn bool(f)\n}\n\nfunc main() {\n\tvar f Flag\n\t_ = f.On()\n}\n"},
		...
		{"errors.As into an interface target", "package main\n\nimport \"errors\"\n\nfunc main() {\n\tvar err error = errors.New(\"x\")\n\tvar target error\n\t_ = errors.As(err, &target)\n}\n"},
		{"errors.As in a compound condition", "package main\n\nimport \"errors\"\n\ntype myErr struct{}\n\nfunc (e *myErr) Error() string { return \"m\" }\n\nfunc main() {\n\tvar err error = &myErr{}\n\tvar target *myErr\n\tif err != nil && errors.As(err, &target) {\n\t\t_ = target\n\t}\n}\n"},
		...
		{"type switch with an init statement", "package main\n\nfunc main() {\n\tvar i interface{} = 7\n\tswitch v := i; v.(type) {\n\tcase int:\n\t}\n}\n"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
